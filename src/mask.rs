  ...
  pub(crate) fn is_masked(&self, state: &State, pixel: Vector2<isize>, v: Vector2<f64>) -> bool {
    match self {
      ...
      Self::Mod { divisor, remainder } => {
        (pixel.x * state.matrix.nrows() as isize + pixel.y) % *divisor as isize == *remainder as isize
      }
      Self::Rows { nrows, step } => pixel.y % (nrows.saturating_add(*step) as isize) < *nrows as isize,
