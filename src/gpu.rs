      ...
      Rows { nrows, step } => {
        self.gl.uniform1i(
          Some(
            &self
              .gl
              .get_uniform_location(&self.program, "nrows")
              .ok_or("Failed to get `nrows` uniform location")?,
          ),
          *nrows as i32,
        );
        self.gl.uniform1i(
          Some(
            &self
              .gl
              .get_uniform_location(&self.program, "step")
              .ok_or("Failed to get `step` uniform location")?,
          ),
          *step as i32,
        );
      }
